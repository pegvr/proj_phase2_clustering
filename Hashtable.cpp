...
int Hashtable:: SearchBucket(int L, string temp, string temp1, string item, int counter, int *Row, int fi, string method, int radius, ofstream& file, int func, string &neighbour, int columns, Cluster ** centroid, int cluster)
{
    int distance = 0;
    //string neighbor;
    if (fi == -1)       //SearchBucket for Hamming,Cosine and DistanceMatrix
    {
        const char *point = temp.c_str();
        int index = strtol(point, NULL, 2); 
        if (func == 1) array[index].Search(radius, temp1, item, counter, Row, method, file, columns, centroid, cluster);
        ...
        if (func == 1)  array[fi].Search(radius, temp1, item, counter, Row, method, file, 0, centroid, cluster);
        else
        {            
